...
import { CheckCircle, CheckCircle2Icon, CheckIcon, RocketIcon } from 'lucide-react';
...
export function ProtectedPage() {
  ...
  const filteredVideos = useMemo(() => {
    ...
    if (selectedCategory) {
      filtered = filtered.filter((video) => video.snippet.categoryId === selectedCategory);
    }
  ...
  }, [videos, selectedCategory, selectedTag, query]);

  const tags = useMemo(() => {
    const all = videos.flatMap((video) => video.snippet.tags || []).filter((tag) => tag.length > 2);
  ...
  const categories = useMemo(() => {
    const all = videos.flatMap((video) => video.snippet.categoryId);
    const unique = Array.from(new Set(all));
    return unique.map((i) => ({
      id: i,
  ...
  const triggerFetchLikedVideos = async () => {
    ...
    if (data) {
      ...
      if (data.nextPageToken) {
        fetchLikedVideos(data.nextPageToken);
      }
    }
  }
      ...
      <div className="p-6 md:p-8 flex flex-col gap-8">
        {googleAccessToken && (
          <div className='flex flex-col gap-4 items-center'>
            ...
            <button className="retro-button-accent flex items-center justify-center gap-2 sm:gap-4 w-fit" onClick={() => triggerFetchLikedVideos()}>
        ...
        {videos && videos.length > 0 &&
          (
            <div className='flex flex-wrap gap-2 sm:gap-4 items-center justify-center'>
              <button onClick={() => setVideos(allvideos)} className="flex items-center gap-2 p-2 glass-card w-fit">
              ...
              <div className='flex flex-wrap gap-2 sm:gap-4 items-center justify-center'>
                {categories.sort((a, b) => b.count - a.count).map((i) => (
                  <button key={i.name} className="flex items-center gap-2 p-2 glass-card" onClick={() => setSelectedCategory(i.id)}>
              ...
              <div className='flex flex-wrap gap-2 sm:gap-4 items-center justify-center'>
                {tags.sort((a, b) => b.count - a.count).map((tag) => (
                  <button key={tag.name} className="flex items-center gap-2 p-2 glass-card" onClick={() => setSelectedTag(tag.name)}>
