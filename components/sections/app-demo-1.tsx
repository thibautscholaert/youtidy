...
export function AppDemo1() {
  return (
    <div className="relative bg-white/10 dark:bg-white/10 rounded-3xl overflow-hidden">
      <div className="h-10 w-full bg-white/20 dark:bg-white/20 flex items-center gap-2 pl-4">
        <div className="w-3 h-3 rounded-full bg-red-400"></div>
        <div className="w-3 h-3 rounded-full bg-yellow-400"></div>
        <div className="w-3 h-3 rounded-full bg-green-400"></div>
        <div className="ml-4 text-xs text-white opacity-80">YouTidy - Your YouTube Library</div>
      </div>
      ...
      <div className="p-6 md:p-8">
        <div className="flex flex-col md:flex-row gap-8">
          <div className="w-full md:w-1/3 p-4 glass-card">
            <div className="mb-6">
              <div className="text-xl font-jakarta font-bold text-gradient mb-2">YouTidy</div>
                ...
                {[
                  ...
                  <div key={title} className="p-4 glass-card">
                    ...
                    <div className="text-sm text-white/70">
                      {index + 1 + 2 * 10 - (index + 1) * 3 + 97 * ((index % 3) + 1) - 7 * index}{' '}
                      videos
                    </div>
                  </div>
                ))}
                ...
                {['2013', '2015'].map((year, index) => (
                  <div key={year} className="flex items-center gap-4 p-4 glass-card">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-white/70">
                        {index + 4 + 2 * 10 - (index + 4) * 3 + 15 * 10 + (index + 4) * 153} videos
                        from {year}
                      </div>
                    </div>
                  </div>
